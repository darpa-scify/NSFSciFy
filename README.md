...
- `notebooks/filter_dup_data_all.ipynb`: filters the 20K NSF awards data into the final train/validation/test JSONL splits.
- `notebooks/filter_dup_data_matsci.ipynb`: filters the materials-science/DMR awards data into the final train/validation/test JSONL splits.

Training and generation:

- `src/data_utils.py`: loads local JSONL data and formats task prompts.
- `scripts/train.py`: main Unsloth/TRL SFT LoRA training entry point.
- `scripts/gen.py`: generation-only entry point.
- `scripts/bash/train/`: shell launchers for the training runs.

Evaluation:

- `scripts/eval.py`: generation plus metric computation.
- `src/eval_utils.py`: generation helpers plus BERTScore, ROUGE, and BLEU evaluation.
- `src/claims_utils.py`: LLM-judge precision, recall, and F-score for verifiable claims and investigation proposals.
- `src/text_utils.py`, `src/model_utils.py`, `src/openai_request_utils.py`, and `src/vis_utils.py`: support utilities.
- `scripts/bash/eval/`: shell launchers for evaluation runs.
- `notebooks/get_scores.ipynb`, `notebooks/show_results*.ipynb`, and `notebooks/metrics/`: result aggregation and display notebooks.
...
Runnable Hugging Face examples are also available in `notebooks/hf_usage_examples.ipynb`.
...
The copied training/evaluation scripts currently call `src/data_utils.py`, which expects local JSONL files under `data/`:
...
To use Hugging Face directly with the existing scripts, replace the local loader in `src/data_utils.py` with a `load_dataset(...)` call that returns `train`, `validation`, and `test`:
...
The same IDs can be passed directly to `scripts/gen.py` or `scripts/eval.py`.

## Training

Run `scripts/train.py` from the repo root:
...
Use `scripts/gen.py` when you only want predictions:
...
Use `scripts/eval.py` for prediction plus metrics:
...
- `technontech2claimsip`: combined claims and investigation proposals; generation-only support exists, but full scoring is not implemented in `scripts/eval.py`.

Metrics:

- `tech2nontech` uses BERTScore, ROUGE, and BLEU in `src/eval_utils.py`.
- Claims and investigation-proposal tasks use LLM-judge pairwise comparison in `src/claims_utils.py` to compute precision, recall, and F-score.
