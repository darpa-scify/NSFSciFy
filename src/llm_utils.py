...
cache = diskcache.Cache(Path(__file__).parent / ".llms.py.cache1")
...
def load_model(model_name: str, api_key: Optional[str] = None, verbose: bool = False, max_tokens: int = 4096):
    """Attempt to load the model based on the name"""
    if "gpt" in model_name or model_name.startswith("o"):
        return MyOpenAIModel(model_name=model_name, api_key=api_key, verbose=verbose, max_tokens=max_tokens)
    else:
...
@cache.memoize()
def get_llm_output(prompts, model='gpt-4o', verbose=False, max_tokens=4096):
    """
    prompt: str
    images: list of PIL images
    system_prompt: str
    """
    llm = load_model(model, verbose=verbose, max_tokens=max_tokens)
    result = llm(prompts)
    return result